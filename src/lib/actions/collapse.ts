...
export default function collapse(node: HTMLElement, params: CollapseParams): ActionReturn {
  ...
  const setDimensions = (params: CollapseParams) => {
    // re-assign the transition in case it got removed;
    // this can happen when resizing the viewport and the layout changes or something
  ...
  // when the viewport changes, recalculate everything
